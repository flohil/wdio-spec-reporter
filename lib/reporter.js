...
class SpecReporter extends events.EventEmitter {
    ...
    getFailureList (failures, preface) {
        ...
        failures.forEach((test, i) => {
            ...
            const printErr = (err) => {
                const message = err.message.split(/\n/g).map((l) => `${preface} ${this.baseReporter.color('error message', l)}`).join('\n')
                output += `${message}\n`
    ...
    getUnverifiedsList (unverifieds, preface) {
        let output = ''

        unverifieds.forEach((test, i) => {
            const title = typeof test.parent !== 'undefined' ? test.parent + ' ' + test.title : test.title
            output += `${preface.trim()}\n`
            output += preface + ' ' + this.baseReporter.color('error title', `${(i + 1)}) ${title}:`) + '\n'

            const printErr = (err) => {
                const message = err.message.split(/\n/g).map((l) => `${preface} ${this.baseReporter.color('unverified', l)}`).join('\n')
                output += `${message}\n`
            }
    ...
    getSuiteResult (runner) {
        ...
        const unverifieds = stats.getUnverifieds().filter((f) => f.cid === cid || Object.keys(f.runner).indexOf(cid) > -1)
        ...
        output += this.getUnverifiedsList(unverifieds, preface)
